...
*/
package com.basdeo.providercorelib.iso8583;


import com.basdeo.providercorelib.Globals.ProviderGlobals;
import com.basdeo.providercorelib.Log.Logger;
import com.basdeo.providercorelib.iso8583.parse.ConfigParser;
import com.basdeo.providercorelib.iso8583.parse.DateTimeParseInfo;
import com.basdeo.providercorelib.iso8583.parse.FieldParseInfo;
...
public class MessageFactory<T extends IsoMessage> {
    ...
    private ProviderGlobals gpg = ProviderGlobals.getInstance();
    ...
    public void setTimezoneForParseGuide(int messageType, int field, TimeZone tz) {
        ...
        log.appendToLibraryLog("\n" + "Field " + field + " for message type " + messageType + " is not for dates, cannot set timezone");
    }
	...
	throws ParseException, UnsupportedEncodingException {
		...
		if (index == null) {
            log.appendToLibraryLog("\n" + String.format("ISO8583 MessageFactory has no parsing guide for message type %04x [%s]", type, new String(buf)));
			return null;
		}
		...
		for (int i = 1; i < bs.length(); i++) {
			if (bs.get(i) && !index.contains(i+1)) {
                log.appendToLibraryLog("\n" + "ISO8583 MessageFactory cannot parse field " +  i+1 + ": unspecified in parsing guide");
			}
		}
		if (abandon) {
			return m;
			...
			for (Integer i : index) {
				...
				if (bs.get(i - 1)) {
					if (ignoreLast && pos >= buf.length && i.intValue() == index.get(index.size() -1)) {
                        log.appendToLibraryLog("\n" + "Field " + i + " is not really in the message even though it's in the bitmap");
						bs.clear(i - 1);
					} else {
	...
	public void setParseMap(int type, Map<Integer, FieldParseInfo> map) {
        ...
        log.appendToLibraryLog("\n" + String.format("ISO8583 MessageFactory adding parse map for type %04x with fields %s", type, index));
